...
class NN:
	...
	def __init__ (self,learning_rate=0.1):
		...
		self.w_edge = np.random.rand(INPUT_SIZE,WEIGHT_DISTRIBUTION) * np.sqrt(2 / 784)
		...
		self.cedge = np.zeros((1,WEIGHT_DISTRIBUTION))
	...
	def forward(self,x):
		...
		cedge = self.conv_2d(x,self.kernel_edge)
		...
		self.cedge = self.flatten_convolution(cedge).reshape(1,INPUT_SIZE)
		...
		self.zedge = self.cd.dot(self.w_edge)
		# Stack layers
		self.z1 = np.array([self.ze,self.zl,self.zr,self.zu,self.zd,self.zedge]).reshape((1,HIDDEN_LAYER_SIZE))
	...
	def kernel_derivitive(self,dCE,A,stride=1):
		x = int(np.floor((A.shape[0]-(self.kernel.shape[0]))/stride)+1)
		y = int(np.floor((A.shape[1]-(self.kernel.shape[1]))/stride)+1)
		d_kernels = np.zeros((x,y,self.kernel.shape[0],self.kernel.shape[1]))
		for i in range(x):
			for j in range(y):
				d_kernels[i,j] = A[i:i+self.kernel.shape[0],j:j+self.kernel.shape[0]]
		# d_kernels.shape (26,26,3,3)
		grad_kernel = np.einsum('hwij,hw->ij',d_kernels,dCE)
		# kernel gradient
		return grad_kernel
	...
	def backwards(self,x,target):
		...
		# layer 1 = z1 = [ze,zl,zr,zu,zd,zedge]
		delta1_ze = delta1[:,0:WEIGHT_DISTRIBUTION]
		delta1_zl = delta1[:,WEIGHT_DISTRIBUTION:WEIGHT_DISTRIBUTION*2]
		delta1_zr = delta1[:,WEIGHT_DISTRIBUTION*2:WEIGHT_DISTRIBUTION*3]
		delta1_zu = delta1[:,WEIGHT_DISTRIBUTION*3:WEIGHT_DISTRIBUTION*4]
		delta1_zd = delta1[:,WEIGHT_DISTRIBUTION*4:WEIGHT_DISTRIBUTION*5]
		delta1_zedge = delta1[:,WEIGHT_DISTRIBUTION*5:WEIGHT_DISTRIBUTION*6]

		# dC/dw1 = dC/z1*dz1/d_w1
		dwe = self.ce.T @ delta1_ze
		dwl = self.cl.T @ delta1_zl
		dwr = self.cr.T @ delta1_zr
		dwu = self.cu.T @ delta1_zu
		dwd = self.cd.T @ delta1_zd
		dwedge = self.cedge.T @ delta1_zedge

		dCE = delta1_ze @ self.w_edge.T
	
		
		dCE = self.unflatten_convolution(dCE)

		grad_kernel = self.kernel_derivitive(dCE,x)

		# full gradiant
		return dwe,dwl,dwr,dwu,dwd,dwedge,grad_kernel,db1,dw2,db2


	def train(self,x,target):
		self.forward(x)
		dwe,dwl,dwr,dwu,dwd,dwedge,grad_kernel,db1,dw2,db2 = self.backwards(x,target)
		...
		self.w_edge -= dwedge*self.learning_rate
		self.kernel -= grad_kernel*self.learning_rate
	...
	def save_network(self):
		...
		np.savez(FILE_NAME,self.w_e,self.w_l,self.w_r,self.w_u,self.w_d,self.w_edge,self.kernel,self.w_2,self.b_1,self.b_2)
		_ = outfile.seek(0)
	...
	def load_network(self):
		...
		self.w_edge = npzfile['arr_5']
		self.kernel = npzfile['arr_6']
		self.w_2 = npzfile['arr_7']
		self.b_1 = npzfile['arr_8']
		self.b_2 = npzfile['arr_9']
...
if __name__ == "__main__":
	...
	if TRAIN:
		...
		nn.load_network()
		#nn.forward(train_X[10])
		#nn.backwards(train_X[10],train_Y[10])
